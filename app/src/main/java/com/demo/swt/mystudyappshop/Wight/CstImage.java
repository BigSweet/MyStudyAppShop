...
public class CstImage extends RelativeLayout {
    ...
    //单张图的时候处理图片显示宽高参数
    private void resetOneImgLayoutParams(ImageView img, String url) {
        RelativeLayout.LayoutParams lp;
        boolean set = false;
        if (null != url && null != img && null != (lp = (LayoutParams) img.getLayoutParams())) {
            String[] arr = url.split("_");
            if (arr.length == 2) {
                String p = arr[1].replace(".jpg", "").replace(".jpeg", "").replace(".png", "").replace(".gif", "");
                String[] params = p.split("x");
                if (params.length == 2) {
                    int w = NumberUtils.getInt(params[0]);
                    int h = NumberUtils.getInt(params[1]);
                    if (w > 0 && h > 0) {
                        set = true;
                        resetLayoutLayoutParams(img, url, lp, w, h);
                    }
                }
            }
            if (!set) {
                resetLayoutLayoutParams(img, url, lp, 400, 400);
            }
        }

    }

    private void resetLayoutLayoutParams(final ImageView img, String url, RelativeLayout.LayoutParams lp, int w, int h) {
    ...
    private void setImg(final List<String> imgs, final int position, final View view) {
        ...
        if (null != img) {
            int size = imgs.size();
            if (size == 1) {
                resetOneImgLayoutParams(img, imgs.get(position));
            } else if (size == 2) {
                LoadImgUtils.loadImgbreviary2(img, imgs.get(position));
            } else if (size == 4) {
                LoadImgUtils.loadImgbreviary4(img, imgs.get(position));
            } else {
                LoadImgUtils.loadImgbreviaryMulti(img, imgs.get(position));
            }
