...
public class QQNaviView extends LinearLayout {
    ...
    public QQNaviView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        mIconWidth = ta.getDimension(R.styleable.QQNaviView_iconWidth, dp2px(context, 30));
        mIconHeight = ta.getDimension(R.styleable.QQNaviView_iconHeight, dp2px(context, 30));
    ...
    private void moveEvent(View view, float deltaX, float deltaY, float radius){
        ...
        //如果大于临界半径就不能再往外拖了
        if (distance > radius){
            Log.d("swtx",view.getLeft() + (float) (radius * Math.cos(degree))+"swtx");
            Log.d("swty",view.getTop() + (float) (radius * Math.sin(degree))+"swty");
