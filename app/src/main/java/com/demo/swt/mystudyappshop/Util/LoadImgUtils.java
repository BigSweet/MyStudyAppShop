...
import com.demo.swt.mystudyappshop.Wight.NumberUtils;
...
public class LoadImgUtils {
 ...
 /*   static {
    ...
    public static void setContext(Context Context) {
        ...
        Constant.SCREEN_WIDTH = DisplayUtils.getScreenWidth();
        Constant.SCREEN_HEIGHT = DisplayUtils.getScreenHeight();
        Constant.STATUSBAR_HEIGHT=DisplayUtils.getStatusBarHeight();
    }
    /**
     * 获取图片尺寸参数
     *
     ...
     * @return
     */
    public static int[] getParams(String url) {
        if (!TextUtils.isEmpty(url)) {
            String[] arr1 = url.split("\\.");
            int len1;
            if (null != arr1 && (len1 = arr1.length) >= 2) {
                String[] arr2 = arr1[len1 - 2].split("_");
                int len2;
                if (null != arr2 && (len2 = arr2.length) > 0) {
                    String[] arr3 = arr2[len2 - 1].split("x");
                    if (null != arr3 && arr3.length == 2) {
                        int[] params = new int[2];
                        params[0] = NumberUtils.getInt(arr3[0]);
                        params[1] = NumberUtils.getInt(arr3[1]);
                        return params;
                    }
                }
            }
    ...
    public static void loadImageWithThumb(ImageView img, String url) {
        if (null != img) {
            if (!TextUtils.isEmpty(url)) {
                Picasso.with(LoadImgUtils.context).load(url).into(img);
           /*     getIonInstance().build(img)
                        .centerCrop()
                        .placeholder(R.drawable.ic_launcher)
                        .error(R.drawable.ic_launcher)
